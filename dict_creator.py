'''
This script is developed with python 3.10 and it reads a downloaded file and uploads the data to s3 as a json file.
...
# Initializing return url based on retun_url_bool
return_url = data['return_url'] if data['retun_url_bool'] else None
    ...
    for i in range(0, xlsBook.nsheets):
        ...
        for row in range(0, xlsSheet.nrows):
            for col in range(0, xlsSheet.ncols):
                sheet.cell(row=row + 1, column=col + 1).value = xlsSheet.cell_value(
                    row, col) if xlsSheet.cell_value(row, col) != '' else None
...
try:
    # loads 'MICs List by CC' sheet mentioned in config.json
    sheet = workbook[sheet_name]
except KeyError:
    # Stops executing if 'MICs List by CC' sheet is missing
    sys.exit('Specified sheet is missing')

data_list = []

# list of all the column names
key_values = [sheet.cell(row=1, column=i).value for i in range(
    1, sheet.max_column+1)]

# Generating dictionary of all the rows with column name as keys
for i in range(2, sheet.max_row+1):
    row_dict = {
        key_values[x-1]: sheet.cell(row=i, column=x).value for x in range(1, sheet.max_column+1)}
    data_list.append(row_dict)


# Saving json data on s3
try:
    time_stamp_string = time.strftime(
        "%Y%m%d-%H%M%S")  # To generate unique filename
    s3.Bucket(bucket_name).put_object(Key='data_' + time_stamp_string +
                                      '.json', Body=json.dumps(data_list, indent=4))
    if return_url:
